...
// *********************************************************************//
  ISpellCheckerFactory = interface(IUnknown)
    ...
    function IsSupported(languageTag: PWideChar; out value: NativeInt): HResult; stdcall;
...
{$REGION 'TSynSpellCheck'}
  ...
  private type
    TWorkItem = record
      Token: string;
      TokenPos: NativeInt;
      constructor Create(AToken: string; ATokenPos: NativeInt);
    end;
  private
  ...
  protected
    ...
    function SpellCheckLine(Editor: TCustomSynEdit; Line: NativeInt;
        StartChar:NativeInt = 0; EndChar: NativeInt = MaxInt; ErrorPos: NativeInt = -1):
        ISpellingError;
  ...
  public
    ...
    procedure EndUpdate;
    procedure Changed(LangId: Boolean = False);
    function AddEditor(AEditor: TCustomSynEdit): NativeInt;
...
function TSynSpellCheck.AddEditor(AEditor: TCustomSynEdit): NativeInt;
var
...
function TSynSpellCheck.SpellCheckLine(Editor: TCustomSynEdit; Line:
    NativeInt; StartChar:NativeInt = 0; EndChar: NativeInt = MaxInt;
    ErrorPos: NativeInt = -1): ISpellingError;
{ The core spell checking function.   Spells checks a whole or part of
  ...
  procedure SpellCheckToken(const Token: string; TokenPos: NativeInt = 0);
  var
  ...
  begin
    ...
    begin
      ...
      if (NativeInt(StartIndex) + 1 < StartChar) then Continue;
      if (NativeInt(StartIndex + Len) > EndChar) then Break;

      if ErrorPos < 0 then
        Editor.Indicators.Add(Line,
          TSynIndicator.Create(SpellErrorIndicatorId,
            NativeInt(StartIndex) + TokenPos + 1,
            NativeInt(StartIndex) + TokenPos + 1 + NativeInt(Len)), False)
...
procedure TSpellCheckPlugin.LinePut(aIndex: NativeInt; const OldLine: string);
var
  Line: string;
  Len1, Len2: NativeInt;
  StartingPos: NativeInt;
begin
...
procedure TSpellCheckPlugin.LinesInserted(FirstLine, Count: NativeInt);
var
  Line: NativeInt;
begin
...
constructor TSynSpellCheck.TWorkItem.Create(AToken: string; ATokenPos: NativeInt);
begin
  Self.Token := AToken;
