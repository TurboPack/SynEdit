...
type
  ...
  private
    ...
    fListColors: TList<TColor>;
    function ColorToRTF(AColor: TColor): string;
    function GetColorIndex(AColor: TColor): NativeInt;
  protected
...
begin
  ...
  fListColors := TList<TColor>.Create;
...
function TSynExporterRTF.GetColorIndex(AColor: TColor): NativeInt;
begin
  Result := fListColors.IndexOf(AColor);
  if Result = -1 then
    Result := fListColors.Add(AColor);
end;
...
begin
  ...
  for i := 0 to fListColors.Count - 1 do
    Result := Result + ColorToRTF(fListColors[i]);
  Result := Result + '}'#13#10;
