...
Ported from Vcl.SynEditUndo.pas — undo logic is platform-independent.
Only the editor type references differ (TCustomFMXSynEdit vs TCustomSynEdit).
-------------------------------------------------------------------------------}
...
type
  ...
  TSynUndoItem = class abstract(TObject)
    ...
    GroupBreak: Boolean;
  public
    procedure Undo(Editor: TCustomFMXSynEdit); virtual; abstract;
    procedure Redo(Editor: TCustomFMXSynEdit); virtual; abstract;
  end;
  ...
  public
    ...
    constructor Create(Editor: TCustomFMXSynEdit; Index: Integer;
  ...
  public
    procedure Undo(Editor: TCustomFMXSynEdit); override;
    procedure Redo(Editor: TCustomFMXSynEdit); override;
    constructor Create(Editor: TCustomFMXSynEdit; Index, Count: Integer);
  end;
  ...
  public
    constructor Create(AOwner: TSynEditUndo);
...
constructor TSynEditUndoList.Create(AOwner: TSynEditUndo);
begin
...
procedure TSynEditUndo.BeginBlock(Editor: TObject);
begin
  ...
  begin
    ...
    FBlockSelRestoreItem := TSynCaretAndSelectionUndoItem.Create(
...
procedure TSynEditUndo.EndBlock(Editor: TObject);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Item := TSynCaretAndSelectionUndoItem.Create(
          Editor as TCustomFMXSynEdit);
        Item.ChangeNumber := FBlockChangeNumber;
        FUndoList.Push(Item);
      end;
...
procedure TSynEditUndo.Undo(Editor: TObject);
var
...
begin
  ...
  begin
    ...
    if not (LastItem is TSynCaretAndSelectionUndoItem) then
      Ed.SetCaretAndSelection(LastItem.FCaret, LastItem.FCaret, LastItem.FCaret);
...
procedure TSynEditUndo.Redo(Editor: TObject);
var
  Item, LastItem: TSynUndoItem;
  OldChangeNumber: Integer;
  OldModified: Boolean;
...
begin
  ...
  begin
    Item := TSynLinePutUndoItem.Create(FEditor, Index, OldLine,
      FCommandProcessed);
    AddUndoItem(Item);
  end;
end;
...
begin
  ...
  if Count > 0 then
  begin
    Item := TSynLinesInsertedUndoItem.Create(FEditor, Index, Count);
    AddUndoItem(Item);
  end;
end;
...
constructor TSynLinesDeletedUndoItem.Create(Editor: TCustomFMXSynEdit;
...
procedure TSynLinesDeletedUndoItem.Undo(Editor: TCustomFMXSynEdit);
var
  I: Integer;
begin
...
procedure TSynLinesDeletedUndoItem.Redo(Editor: TCustomFMXSynEdit);
var
  I: Integer;
begin
...
constructor TSynLinesInsertedUndoItem.Create(Editor: TCustomFMXSynEdit;
  Index, Count: Integer);
var
  I: Integer;
begin
...
procedure TSynLinesInsertedUndoItem.Undo(Editor: TCustomFMXSynEdit);
var
  I: Integer;
begin
  SetLength(FChangeFlags, Length(FLines));
...
procedure TSynLinesInsertedUndoItem.Redo(Editor: TCustomFMXSynEdit);
var
  I: Integer;
begin
  TSynEditStringList(Editor.Lines).InsertStrings(FIndex, FLines);
...
constructor TSynLinePutUndoItem.Create(Editor: TCustomFMXSynEdit;
  Index: Integer; const OldLine: string; Command: TSynEditorCommand);
var
  Len1, Len2: Integer;
  Line: string;
begin
...
procedure TSynLinePutUndoItem.Undo(Editor: TCustomFMXSynEdit);
var
  Line: string;
  Char: Integer;
...
procedure TSynLinePutUndoItem.Redo(Editor: TCustomFMXSynEdit);
var
  Line: string;
  Char: Integer;
  TempCF: TSynLineChangeFlags;
